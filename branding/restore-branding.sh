...
if ! grep -q 'mp-lang-picker' "$HTML"; then
    ...
    <style>
      ...
      .mp-top-btn{display:flex;align-items:center;justify-content:center;width:32px;height:32px;border-radius:8px;border:1px solid var(--border-primary,#333);background:var(--bg-secondary,#1a1a1a);color:var(--text-primary,#fff);cursor:pointer;font-size:16px;transition:all .2s;margin-right:4px}
      .mp-top-btn:hover{border-color:#FF0080;background:#FF0080;color:#fff}
    </style>
    <script>
    (function(){
      var LANGS={en:"English","zh-CN":"\u4e2d\u6587(\u7b80)","zh-TW":"\u4e2d\u6587(\u7e41)","pt-BR":"Portugu\u00eas",de:"Deutsch",es:"Espa\u00f1ol",it:"Italiano"};
      var DEFAULT_GATEWAY_TOKEN="__MP_DEFAULT_GATEWAY_TOKEN__";
      ...
      function getGatewayToken(){
        try{
          var direct=localStorage.getItem("openclaw-gateway-token");
          if(direct)return direct;
          var settings=localStorage.getItem("openclaw-settings");
          if(settings){
            var parsed=JSON.parse(settings);
            if(parsed&&parsed.gatewayToken)return parsed.gatewayToken;
            if(parsed&&parsed.auth&&parsed.auth.token)return parsed.auth.token;
          }
        }catch(e){}
        if(DEFAULT_GATEWAY_TOKEN)return DEFAULT_GATEWAY_TOKEN;
        return null;
      }
      ...
      function injectModelBtn(){
        ...
        var btn=document.createElement("button");btn.className="mp-top-btn mp-model-btn";btn.title="Gestione Modelli AI";btn.innerHTML="\ud83e\udde0";
        btn.onclick=function(){
          var token=getGatewayToken();
          var url="/docs-moneypenny/model-manager.html";
          if(token){
            try{localStorage.setItem("openclaw-gateway-token",token)}catch(e){}
            url+=url.indexOf('?')===-1?"?token="+encodeURIComponent(token):"&token="+encodeURIComponent(token);
          }
          window.open(url,"_twiza_models","width=1100,height=800");
        };
        topbar.parentNode.insertBefore(btn,topbar);
      }
      function injectDocsBtn(){
        var topbar=document.querySelector(".topbar-status");
        if(!topbar||!topbar.parentNode)return;
        if(topbar.parentNode.querySelector(".mp-docs-btn"))return;
        var btn=document.createElement("button");btn.className="mp-top-btn mp-docs-btn";btn.title="Documentazione";btn.innerHTML="\ud83d\udcda";
        btn.onclick=function(){window.open("/docs-moneypenny/index.html","_twiza_docs","width=1100,height=800")};
        topbar.parentNode.insertBefore(btn,topbar);
      }
      function inject(){injectPicker();injectDocsBtn();injectModelBtn()}
      inject();
      new MutationObserver(function(){inject()}).observe(document.body,{childList:true,subtree:true});
    })();
    </script>
HTMLEOF
    MP_DEFAULT_GATEWAY_TOKEN="$GW_TOKEN" python3 - "$HTML" <<'PYEOF'
import os, sys
path = sys.argv[1]
token = os.environ.get('MP_DEFAULT_GATEWAY_TOKEN', '')
with open(path, 'r', encoding='utf-8') as f:
    data = f.read()
data = data.replace('__MP_DEFAULT_GATEWAY_TOKEN__', token)
...
# --- 8. Patch CSP for Ollama connect + inline scripts (docs pages) ---
echo "🔌 [8/8] Patching CSP..."
GW_JS=$(find "$OPENCLAW_PKG/dist" -name 'gateway-cli-*.js' -not -name '*.bak' 2>/dev/null | head -1)
if [ -n "$GW_JS" ] && [ -f "$GW_JS" ]; then
    # Patch connect-src for Ollama
    if grep -q "connect-src 'self' ws: wss:\"" "$GW_JS"; then
        sed -i "s|connect-src 'self' ws: wss:\"|connect-src 'self' ws: wss: http://localhost:11434 http://127.0.0.1:11434\"|" "$GW_JS"
        echo "  ✅ CSP connect-src patched for Ollama"
    elif grep -q "http://localhost:11434" "$GW_JS"; then
        echo "  ✅ CSP connect-src already patched"
    fi
    # Patch script-src to allow inline scripts (docs-moneypenny i18n)
    if grep -q "script-src 'self'\"" "$GW_JS"; then
        sed -i "s|script-src 'self'\"|script-src 'self' 'unsafe-inline'\"|" "$GW_JS"
        echo "  ✅ CSP script-src patched for inline scripts"
    elif grep -q "'unsafe-inline'" "$GW_JS"; then
        echo "  ✅ CSP script-src already patched"
    fi
else
    echo "  ⚠️ Gateway JS not found — manual CSP patch needed"
fi
